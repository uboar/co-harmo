#!/usr/bin/env bash
# M4 smoke test: exercises summarize_clip and diff_clip_abc logic.
# Builds all three components (unless M2_SKIP_BUILD=1), launches the Standalone
# plugin, waits for bridge.json, runs m4-probe.mjs, then tears down.
set -euo pipefail
...
PROBE="$REPO/scripts/m4-probe.mjs"
